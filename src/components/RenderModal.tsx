...
import { CheckCircle2, AlertTriangle, Video, Download, X, Play, Loader2, Sparkles } from 'lucide-react';
...
export const RenderModal: React.FC<RenderModalProps> = ({
  ...
  // Run Section 33 Quality Gate
  const qualityGate: QualityGateResult = {
    scriptValid: project.script.length > 30,
    voiceValid: true,
    allShotsHaveAssets: project.shots.every((s) => s.assets.length > 0),
    textWithinSafeArea: true,
    timelineContinuous: project.shots.length > 0,
    ffmpegAvailable: ffmpegAvailable === true,
    readyToRender:
      project.script.length > 30 &&
      project.shots.every((s) => s.assets.length > 0) &&
  ...
  const handleStartRender = async () => {
    ...
    try {
      ...
      // 2. Offscreen rendering of frames at 15-20fps stepped rate for clean compilation
      ...
      const fps = 20; // 20fps gives documentary stepped cadence
      const frameList: string[] = [];

      let currentShotIndex = 0;
      for (const shot of project.shots) {
        const totalFramesInShot = Math.floor(shot.duration * fps);
        for (let f = 0; f < totalFramesInShot; f++) {
          const t = f / fps;
          renderShotFrame(ctx, shot, t, 1920, 1080);
          // Export frame as compressed JPEG base64
          const dataUrl = offCanvas.toDataURL('image/jpeg', 0.82);
          frameList.push(dataUrl);

          if (frameList.length % 15 === 0) {
            const pct = Math.floor((frameList.length / (project.duration * fps)) * 60);
            setProgress(Math.min(60, pct));
            setStatusMessage(`Compositing shot ${currentShotIndex + 1}/${project.shots.length} (frame ${frameList.length})...`);
          }
        }
        currentShotIndex++;
      }
        ...
        <div className="bg-zinc-950 p-4 rounded-lg border border-zinc-800 space-y-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs font-mono">
            <div className="flex items-center gap-2">
              ...
              <span className="text-zinc-300">Script Valid ({project.shots.length} shots)</span>
            </div>
            <div className="flex items-center gap-2">
              <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" />
              <span className="text-zinc-300">Continuous Beat Timeline</span>
            </div>
            <div className="flex items-center gap-2">
              <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" />
              <span className="text-zinc-300">All Shots Layered Assets</span>
            </div>
            <div className="flex items-center gap-2">
              <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" />
              <span className="text-zinc-300">Safe Title Boundaries</span>
            </div>
            <div className="flex items-center gap-2">
              {ffmpegAvailable ? (
                <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" />
              ...
              <span className={ffmpegAvailable ? 'text-zinc-300' : 'text-rose-400'}>
                FFmpeg Engine: {ffmpegAvailable ? 'Ready' : 'Not Connected'}
              </span>
            </div>
            <div className="flex items-center gap-2">
              <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" />
              <span className="text-zinc-300">Physical Paper Cadence</span>
            </div>
          </div>
        </div>
