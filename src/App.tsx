...
export default function App() {
  ...
  useEffect(() => {
    fetch('/api/health')
      .then((res) => res.json())
      .then((data) => setFfmpegAvailable(data.ffmpeg === true))
  ...
  const handleGenerateStoryboard = async () => {
    ...
    try {
      const response = await fetch('/api/director/storyboard', {
        ...
        body: JSON.stringify({ topic, duration, niche: 'documentary' }),
      });
  ...
  const handleRegenerateShotLayout = (shotIndex: number) => {
    setProject((prev) => {
      ...
      shots[shotIndex] = { ...shots[shotIndex], layout: layoutIds[(currentIndex + 1) % layoutIds.length] };
      return { ...prev, shots };
    });
  };

  const handleRegenerateShotMotion = (shotIndex: number) => {
    const paperMotions: MotionID[] = ['paper_drop','paper_slide_left','paper_slide_right','paper_slide_up','photo_stack','paper_reveal'];
    setProject((prev) => {
      const shots = [...prev.shots];
      ...
      const nextMotion = paperMotions[(paperMotions.indexOf(currentMotion) + 1) % paperMotions.length];
      shots[shotIndex] = { ...targetShot, assets: targetShot.assets.map((a) => a.role === 'hero' ? { ...a, motion: nextMotion } : a) };
      return { ...prev, shots };
    });
  };
  ...
  const handleGenerateShotAsset = async (shotIndex: number, assetId: string) => {
    ...
    const prompt = asset?.assetPrompt || `Archival vintage document or photograph regarding ${project.title}, beat ${shot.order}`;
    setProject((prev) => ({ ...prev, shots: prev.shots.map((s, i) => i === shotIndex ? { ...s, assets: s.assets.map((a) => a.id === assetId ? { ...a, status: 'generating' } : a) } : s) }));
    try {
      const response = await fetch('/api/assets/generate', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ prompt, topic:project.title, shotId:shot.shot_id, assetId, role:asset?.role||'hero', layout:shot.layout }) });
  ...
  const handleUploadShotAsset = async (shotIndex: number, assetId: string, file: File) => {
    ...
    reader.onload = async () => {
      try {
        const response = await fetch('/api/assets/upload', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ imageBase64:reader.result as string, shotId:project.shots[shotIndex].shot_id, assetId }) });
        const result = await response.json();
        if (result.success && result.url) setProject((prev) => ({ ...prev, shots: prev.shots.map((s,i) => i===shotIndex ? { ...s, assets:s.assets.map(a => a.id===assetId ? { ...a, source:result.url, provider:'upload', status:'ready', paperCutout:true } : a) } : s) }));
      } catch (err) { console.error('Upload asset error:', err); }
      finally { setGeneratingAssetKey(null); }
    };
  ...
  const handleGenerateVoiceAndTimeline = async (voiceId?: string, provider?: TTSProviderChoice, language?: string) => {
    ...
    const chosenLanguage = language || selectedTTSLanguage;
    try {
      const response = await fetch('/api/voice-and-timeline', {
        method:'POST', headers:{'Content-Type':'application/json'},
        body:JSON.stringify({ project, provider:chosenProvider, voiceId:chosenVoice, lang:chosenLanguage }),
      });
      if (!response.ok) { const errData=await response.json().catch(()=>({})); throw new Error(errData.error||'Failed to synthesize voice and beat timeline'); }
      const data=await response.json();
      if (data.success && data.project) {
        setProject(data.project); setDuration(data.project.duration);
        setVoiceNotice(lang==='vi' ? `Đã tạo giọng đọc và đồng bộ ${data.project.audioTimeline?.beats?.length||0} nhịp phân cảnh thành công!` : `Audio-first timeline synced successfully (${data.project.audioTimeline?.beats?.length||0} beats).`);
      }
    } catch (err:any) {
      setVoiceNotice(lang==='vi' ? `Thông báo: ${err.message}` : `TTS notice: ${err.message}`);
    ...
    <div className="h-screen w-screen overflow-hidden bg-[#121214] text-[#E4E4E7] font-sans-body flex flex-col selection:bg-red-900 selection:text-white">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-hidden bg-[#18181B]/50">
          ...
          {activeTab==='overview' && <ProjectOverviewView project={project} topic={topic} setTopic={setTopic} duration={duration} setDuration={setDuration} onGenerateStoryboard={handleGenerateStoryboard} isGenerating={isGenerating} onJumpToCanvas={() => setActiveTab('canvas')} lang={lang} />}
