...
export function runQualityGate(project: Project, ffmpegAvailable: boolean): QualityGateResult {
  ...
  for(const shot of shots){const start=Number(shot.start.toFixed(3));const end=Number(shot.end.toFixed(3));const duration=Number(shot.duration.toFixed(3));if(start<0||end<=start||Math.abs(end-start-duration)>0.3)timelineContinuous=false;if(Math.abs(start-cursor)>0.3)timelineContinuous=false;cursor=end;}
