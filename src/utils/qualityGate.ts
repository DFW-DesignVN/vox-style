...
export function runQualityGate(project: Project, ffmpegAvailable: boolean): QualityGateResult {
  ...
  const textWithinSafeArea = shots.every((shot) =>
    shot.text.every(
      (text) =>
        text.position.x >= SAFE_MIN &&
        text.position.x <= SAFE_MAX &&
        text.position.y >= SAFE_MIN &&
        text.position.y <= SAFE_MAX
    )
  );
  ...
  for (const shot of shots) {
    ...
    if (start < 0 || end <= start || Math.abs(end - start - duration) > 0.12) {
      timelineContinuous = false;
    }
    if (Math.abs(start - cursor) > 0.12) {
      timelineContinuous = false;
    }
    cursor = end;
  }

  if (Math.abs(cursor - project.duration) > 0.2) {
    timelineContinuous = false;
  }
  if (!timelineContinuous) {
    messages.push('Shot timeline has gaps, overlaps, or does not match project duration.');
  }
  ...
  if (!voiceValid) {
    messages.push('Voiceover is enabled but no voice file is attached yet; V0.1 will render visuals only.');
  }
  ...
  if (!ffmpegAvailable) {
    messages.push('FFmpeg is not available.');
  }
