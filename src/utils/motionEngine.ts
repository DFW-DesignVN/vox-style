...
export interface MotionTransform { offsetX:number; offsetY:number; scale:number; scaleX?:number; scaleY?:number; rotation:number; opacity:number; progress:number; revealedChars?:number; }
...
export function calculateMotionTransform(motion:MotionID,shotTime:number,startTime:number,textLength=0):MotionTransform{
 const elapsed=shotTime-startTime;if(elapsed<0)return{offsetX:0,offsetY:0,scale:0,rotation:0,opacity:0,progress:0,revealedChars:0};
 const t=(d:number)=>Math.min(1,Math.max(0,elapsed/d));const stepped=(d:number,s=12)=>steppedEase(t(d),s);const settle=(v:number,o=1.06)=>overshootSettle(v,o);
 switch(motion){
  case 'paper_flip':{const p=t(.72),e=stepped(.72,18),angle=e*Math.PI;return{offsetX:0,offsetY:-18*Math.sin(angle),scale:.99+.01*Math.sin(angle),scaleX:Math.max(.035,Math.abs(Math.cos(angle))),scaleY:1,rotation:Math.sin(angle)*1.2,opacity:Math.min(1,p*5),progress:p};}
