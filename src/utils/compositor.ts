...
export function renderShotFrame(
  ...
  for (const asset of shot.assets) {
    ...
    if (img && img.complete && img.naturalWidth > 0) {
      ...
      // Procedural archival placeholder illustration if image loading or custom
      ctx.save();
      ctx.fillStyle = '#2A2A2A';
      ctx.fillRect(-halfW + photoPad, -halfH + photoPad, photoW, photoH);

      // Faint archival lines & label
      ctx.fillStyle = '#E5E5E5';
      ctx.font = 'bold 22px "Courier Prime", monospace';
      ctx.textAlign = 'center';
      ctx.fillText(`ARCHIVAL RECORD [${asset.id.toUpperCase()}]`, 0, -20);
      ctx.font = '16px "Courier Prime", monospace';
      ctx.fillStyle = '#A1A1AA';
      ctx.fillText(asset.role.toUpperCase() + ' CUTOUT', 0, 15);
      ctx.restore();
    }
